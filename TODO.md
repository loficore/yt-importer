...
- [x] CSV 验证工具（导入前检查文件格式）
  - [x] 检测重复 Track URI
  - [x] 询问用户去重方式（保留第一个/最后一个/不处理）
  - [x] 生成去重后的 CSV 文件
...
- [x] 继续提取其他模块硬编码文本
- [x] 新增国际化扫描脚本 (scripts/scan-i18n.ts)
- [x] 修复 settingsHandlers.ts 硬编码
- [x] 修复 config.ts 硬编码
- [x] 添加中英日三种语言翻译
...
- [x] CSV 验证工具（导入前检查文件格式）
- [x] 主菜单添加杂项工具入口
- [x] 主菜单添加 Tools 菜单项（中英日）
- [x] CSV 验证结果美化输出
- [x] 健康检查命令（检查 cookies、代理、网络）- 部分实现
- [x] 批量重试失败曲目（多策略搜索 + 多选界面）
...
- [x] CSV 验证工具输出被清空问题（添加按 Enter 继续提示）
